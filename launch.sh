...
if ! [[ -f serverstarter-B13.0.jar ]]; then
	...
	if command -v wget &> /dev/null; then
		...
		wget -O serverstarter-B13.0.jar "${URL}"
	elif command -v curl &> /dev/null; then
		echo "DEBUG: (curl) Downloading ${URL}"
		curl -o serverstarter-B13.0.jar "${URL}"
	else
...
java $JVM_OPTS -Dlog4j.configurationFile=log4j2_112-116.xml -jar serverstarter-B13.0.jar nogui
